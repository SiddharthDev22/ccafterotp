...
<head>
    ...
    <title>Computer Centre Inventory Management System(CCIMS)</title>
    <!-- Bootstrap CSS -->
...
<h3 class="heading">Stock Issuing Entry</h3>
  <div class="centerd">
<div class="concontent-item-fetch"></div>
	...
	<div class="btn-group btn-group-toggle" data-toggle="buttons" style="height:38px;">
		...
		<label class="btn btn-secondary">
			<input type="radio" name="options" onchange="flitertheitem(this)" value="Equipments">Equipments
		</label>
		<label class="btn btn-secondary">
	...
	function flitertheitem(itemradio){
		...
		window.location.replace('stockIssue.php?inventory='+a);
	}
	</script>
...
?>
            ...
            <a href="" id="url">
            <button type="button" class="btn btn-primary btn-lg" onclick="return myscheme()">Issue</button></a>
            </div>
            </div>
            </div>
            </div>
...
<script>
    function myscheme(){
            ...
            document.getElementById('url').href='stockIssueForm.php?scheme='+scheme+'&category='+sentItem;
